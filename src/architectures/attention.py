...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        x = x.unsqueeze(1)
        weights = torch.matmul(x.transpose(-1, 1), x)
        norm_weights = F.softmax(weights, dim=-1)
        y = torch.matmul(x, norm_weights.transpose(-1, 1))
        return y.squeeze(1)
...
class AttentionConv(nn.Module):
    def __init__(
        ...
        ), "out_channels should be divided by groups. (example: out_channels: 40, groups: 4)"  # noqa
